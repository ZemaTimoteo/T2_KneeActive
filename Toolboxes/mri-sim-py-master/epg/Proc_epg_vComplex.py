...
if PC == 0:
    ...
    from epg_cpmg_LsB_vComplex import epg_cpmg_LsB_vComplex

def Proc_epg_vComplex(exc_pulse, refoc_pulse, exc_phase, refoc_phase, T1, T2, dTE, ETL):
    """
    Input:
    ...
    echoes = np.zeros((ETL,exc_pulse.shape[0]), dtype=np.complex64) # % zero matrix ETL * Length exc_pulse
    soma   = np.zeros( ( ETL, int((T2.shape)[0]),  exc_phase.shape[0] ), dtype=np.complex64)
    ...
    for j in range( exc_phase.shape[0] ):
        for i in range( int((T2.shape)[0]) ):
            for z in range(  exc_pulse.shape[0] ): # 1 -->numero de TR(128)
                print('   -> ShapeRF ', z+1, ' / ', int(exc_pulse.shape[0]), ' | T2_i:', i+1, '/', int((T2.shape)[0]), ' tests')
                # s = epg_cpmg_LsB(exc_pulse[z], exc_phase[j], refoc_pulse[z,:], ETL, T1, T2[i], dTE, 0, refoc_phase)
                s = epg_cpmg_LsB_vComplex(exc_pulse[z], exc_phase[j], refoc_pulse[z], ETL, T1, T2[i], dTE, 0, refoc_phase)
    ...
    sz2 = int(D1.shape[1])
    sz3 = int(D1.shape[2])
    Dict = np.zeros((ETL, sz3 * sz2), dtype=np.complex64)
