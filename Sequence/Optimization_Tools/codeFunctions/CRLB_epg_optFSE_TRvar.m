function [CRLB_num,uCRLB,test_vardT2] = CRLB_epg_optFSE_TRvar(S, phi, refoc_phase, B1, T1, T2, dTE, ETL, TRacq, Trec, flipA, sigma, step, maxETL, plotTest,dir_data)
% function [CRLB_num,uCRLB,FIM_numT2] = CRLB_epg_optFSE_TRvar(S, phi, refoc_phase, B1, T1, T2, dTE, ETL, TRacq, Trec, flipA, sigma, step, maxETL, plotTest,dir_data)
...
% uCRLB = (abs(CRLB_num(2))/(T2^2));    % variance CRLB / s - in Zhang et al. 2013 in Proc. EMBS

% test_vardT2 = ( (dS_dT2(:)'*dS_dT2(:)) / (sigma^2) ) * (TRacq);
test_vardT2 = ( (dS_dT2(:)'*dS_dT2(:)) / (sigma^2) ) / (TRacq);
