function Dict = auxOptimiz_my_epg_ASM(exc_pulse,refoc_pulse,exc_phase,refoc_phase,T1,T2,dTE,ETL,Trec)
...
for j=1:length(exc_phase)
    for i=1:length(T2)
        ...
        for z=1:length(exc_pulse) % 1-->pontos dos pulsos (128)
            s = auxOptimiz_epg_cpmg_ASM(exc_pulse(z),exc_phase(j),refoc_pulse(z,:),ETL,T1,T2(i),dTE,0,refoc_phase);
            echoes(:,z) = s;
        end
