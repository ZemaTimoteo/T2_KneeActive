function [dictionary,pars] = auxOptimiz_dict_pars_generator_ASM(T1,T2,B1,ESP, ETL,refoc_phase,phase_exc,FA_exc,FA_refoc,dir_data,methodDic)
...
if methodDic == 'DICT_SLR'
    ...
    for jj=1:size(B1,2)
        ...
        Dict = auxOptimiz_my_epg_ASM(exc_pulse,refoc_pulse,phase_exc,refoc_phase,T1,T2,ESP,ETL,Trec);
...
elseif methodDic == 'JUST_DIC'
    ...
    LargePhant_dict = zeros(ETL,length(T2)*length(phase_exc)*length(B1));
    FA_refoc = FA_refoc*pi/180;
    % Loop over B1 scale values
    for i=1:length(B1)
        exc_pulse(i) = FA_exc*B1(i);
        refoc_pulse  = FA_refoc*B1(i);
        Dict = auxOptimiz_my_epg_ASM(exc_pulse(i),refoc_pulse,phase_exc,refoc_phase,T1,T2,ESP,ETL);
        % add the entry to LargePhant_dic
