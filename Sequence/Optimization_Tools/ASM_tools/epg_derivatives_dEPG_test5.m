function [ds_dT2,ds_dFl,epg] = epg_derivatives_dEPG_test5(alpha_RF , alpha, n, nRates, R1, R2vec, tau, S, Trec, T1, ETL, aux2var_S_T2_step)             
...
for iRate=1:nRates % loop over different relaxation rates
    ...
    x = E*x;
    epg(1,iRate) = x(1).*aux2var_S_T2_step;
    
% %     aux2var_S_T2 = (1 - auxVariab_S_T2) / ( 1 - auxVariab_S_T2 * S_test(ETL) );
% %     S_test       = aux2var_S_T2.* S_test;    
...
% %         x = full_S*x;
        ...
        epg(i,iRate) = x(1).*aux2var_S_T2_step;
    end
