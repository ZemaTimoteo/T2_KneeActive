function [ds_dT2,ds_dFl,x] = epg_derivatives_dEPG_test3(alpha_RF , alpha, n, nRates, R1, R2vec, tau, S)
...
for iRate=1:nRates % loop over different relaxation rates
    ...
    RArray    = cell(1,n);
    RArray(:) = {sparse(R0)};
    R         = blkdiag(RArray{:}); % takes into account the config state of RFexc
    RArray(:) = {sparse(R0d)};
    Rd        = blkdiag(RArray{:});
