function [ds_dT2,ds_dFl,x] = epg_derivatives_dEPG_test2(alpha_RF , alpha, n, nRates, R1, R2vec, tau, S)
...
T0 = [ cos(alpha/2)^2, sin(alpha/2)^2, -sin(alpha); ...
      ...
      -0.5*sin(alpha), 0.5*sin(alpha),  cos(alpha)];  % pq é que nao multiplica por 'i', pq é que nao multiplica por exp? Mx, My, Mz - pois a phase é zero - CPMG
...
% 1.3 - RF mixing matrix, T (Rotation Matrix)
p      = pi/2;
T0_exc = [ cos(alpha_RF/2)^2                 , exp(2*1i*p)*(sin(alpha_RF/2)).^2 , -1i*exp(1i*p)*sin(alpha_RF); ...
           exp(-2*1i*p)*(sin(alpha_RF/2)).^2 , cos(alpha_RF/2)^2                ,  1i*exp(-1i*p)*sin(alpha_RF); ...
          -0.5*1i*exp(-1i*p)*sin(alpha_RF)   , 0.5*exp(1i*p)*sin(alpha_RF)      ,  cos(alpha_RF)];  % pq é que nao multiplica por 'i', pq é que nao multiplica por exp? Mx, My, Mz - pois a phase é zero - CPMG

% 1.2 - Derivative of T with respect to alpha
T0d_exc = [-0.5*sin(alpha_RF)               ,  0.5*exp(2*1i*p)*sin(alpha_RF) , -1i*exp(1i*p)*cos(alpha_RF);  ...
            0.5*exp(-2*1i*p)*sin(alpha_RF)  , -0.5*sin(alpha_RF)             ,  1i*exp(-1i*p)*cos(alpha_RF); ...
           -0.5*1i*exp(-1i*p)*cos(alpha_RF) , 0.5*exp(1i*p)*cos(alpha_RF)    , -sin(alpha_RF)];
   
% 1.3 - Build mixing matrix for all states
TArray    = cell(1,n);
TArray(:) = {sparse(T0)};
% % T         = blkdiag(1,TArray{:});  % sin alfa
T         = blkdiag(T0_exc,TArray{:}); % sin alfa
TArray(:) = {sparse(T0d)};
Td        = blkdiag(T0d_exc,TArray{:});
...
for iRate=1:nRates % loop over different relaxation rates
    ...
    % Relaxation matrix
    R2  = R2vec(iRate); % values of T2
    R0  = diag([exp(-tau*R2),exp(-tau*R2),exp(-tau*R1)]); % relaxation matrix
    ...
    RArray    = cell(1,n);
    RArray(:) = {sparse(R0)};
    R         = blkdiag(R0_exc,RArray{:}); % takes into account the config state of RFexc
    RArray(:) = {sparse(R0d)};
    Rd        = blkdiag(R0d_exc,RArray{:});
