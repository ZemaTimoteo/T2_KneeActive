...
function [ obj, grad, FF ] = epg_derivatives_fmincon_vF( ETL, tau, R1, R2vec, alpha_RF, alpha)
...
[aux_ds_dT2(:,z),aux_ds_dFA(:,z),aux_epg,grad] = epg_derivatives_fmincon_dEPG_vF( ...
...
grad.ds_dT2_dTE  = sum(grad.ds_dT2_dTE,2);
grad.ds_dT2_dETL = sum(grad.ds_dT2_dTE,2);
grad.ds_dT2_dFA  = sum(grad.ds_dT2_dFA,2);
