...
function [ obj, grad, FF ] = epg_derivatives_test_dirac_pulse( n, tau, R1, R2vec, alpha_RF, alpha )

%% 1 - Initialization
nRates = length(R2vec); % number of T2 to test
tau    = tau/2;         % Half of echospacing in (s)
...
for z=1:size(alpha,1) % |--> pontos dos pulsos (128)
    ...
    [aux_ds_dT2(:,z),aux_ds_dFl(:,z),aux_epg(:,z)] = epg_derivatives_dEPG_dirac_pulse(alpha_RF, alpha(z), n, nRates, R1, R2vec, tau, S);
