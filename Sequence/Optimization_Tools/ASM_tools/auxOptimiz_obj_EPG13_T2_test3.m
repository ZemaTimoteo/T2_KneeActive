function [obj, grad, FF]  = auxOptimiz_obj_EPG13_T2_test3(params,exc_puls,refoc_puls,ESP,ETL,T1,T2,Adj_stat,B1,frequencies,klim,Trec,Trec_Contrl,testPlot)
...
% % FF = F ./ norm( F );         % normalize dictionary
FF = F;

% 6.3 - Get Objective function
...
for z=1:size(alph_test,1) % |-->pontos dos pulsos (128)
    gR = auxOptimiz_dEPG(alph_test(z,:), ph_test, RS_full, T2, TE, S0, N, S, np, FF);
    soma_dEPG(:,:,z) = gR;
end
