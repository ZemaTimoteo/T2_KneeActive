function [obj, grad, FF]  = auxOptimiz_obj_EPG13_T2_test5(params,exc_puls,refoc_puls,ESP,ETL,T1,T2,Adj_stat,B1,frequencies,klim,Trec,Trec_Contrl,testPlot)
...
%     plot(abs(Matrix_dS_dT2(1,size(Matrix_dS_dT2,2)-(size(alph,2)-2)+1:end)))
    aux_grad_Plot = Matrix_dS_dT2(1,size(Matrix_dS_dT2,2)-(size(alph,2)-2)+1:end);
    grad_Plot =  aux_grad_Plot ./ norm( aux_grad_Plot );        
    plot(abs(grad_Plot))
