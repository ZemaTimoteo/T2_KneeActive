...
function [ obj, grad, FF ] = epg_derivatives_test2( n, tau, R1, R2vec, alpha_RF, alpha )
...
S(2,3) = 1; % finally F1+ - relates to F-1-
S(2,1) = 1; % 
S(3,6) = 1;
S(4,4) = 1; % Z states
...
s_Full = full(S);
aux_s  = s_Full(2:end,2:end);
% ----- new S
newS = blkdiag(zeros(3,3),aux_s);


%% 3 ==== Cicle for derivatives for points per RF pulse ----
for z=1:size(alpha,1) % |--> pontos dos pulsos (128)
        
    [aux_ds_dT2(:,z),aux_ds_dFl(:,z),aux_x(:,z)] = ...
        epg_derivatives_dEPG_test2(alpha_RF(z), alpha(z), n, nRates, R1, R2vec, tau, newS);
