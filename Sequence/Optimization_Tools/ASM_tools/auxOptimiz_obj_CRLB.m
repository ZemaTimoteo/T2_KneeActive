function [obj, grad, FF, aux_grad]  = auxOptimiz_obj_CRLB(params,ESP,T1,T2,c,B1,frequencies,klim,sigma)
...
for ns = 1:Ns % loop over space
    ...
    for jj = 2:np+1 % loop over number of different states
        ...
        TE = ESP;   % EchoTime for derivative
        ...
        if jj == 3 % first refocusing
            ...
            GRAD(np+jj-1,ns) = dpdyj*TEMP1p;
        else       % excitation & other refocusing
            GRAD(jj-1,ns)    = dadxj*TEMP2a;
            GRAD(np+jj-1,ns) = dpdyj*TEMP2p;
        end        
    end
end

aux_grad = real(sum(GRAD,2));
aux_grad = aux_grad(:);

%% CRLB fisher matrix from T2 (Tamir, 2021)

grad = - aux_grad(:)'*aux_grad(:).*(1/sigma^2);   %% é a minha funçao objetivo
% vou ter de arranjar a derivada em funcao dos meus parametros de troca
