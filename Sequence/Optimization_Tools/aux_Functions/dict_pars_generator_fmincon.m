function [dictionary,pars] = dict_pars_generator_fmincon(T1,T2,B1,ESP, ETL,refoc_phase,phase_exc,FA_exc,FA_refoc)
...
% %     f = waitbar(jj/size(B1,2),'Building EPGs...');
    ...
    for ii=1:size(FA_refocUNIQUE,2)
        [exc_pulse, refoc_pulseSLR] = slr_profile_fmincon(B1(jj),FA_refocUNIQUE(ii));   % corrige com os perfis adaptar para os pulsos da Siemens/pypulseq
        refoc_pulse = [refoc_pulse refoc_pulseSLR(:)];
        clear refoc_pulseSLR
    end
        
    Dict = my_epg_fmincon(exc_pulse,refoc_pulse,phase_exc,refoc_phase,T1,T2,ESP,ETL);
