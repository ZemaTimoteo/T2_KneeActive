function Dict = my_epg_TRvar_test(exc_pulse,refoc_pulse,exc_phase,refoc_phase,T1,T2,dTE,ETL,Trec)
...
% %         disp('T2: '),i
        ...
        for z=1:length(refoc_pulse) % 1-->pontos dos pulsos apenas para o RF refocusing (128)
            s = epg_cpmg(exc_pulse,exc_phase(j),refoc_pulse(z,:),ETL,T1,T2(i),dTE,0,refoc_phase);
