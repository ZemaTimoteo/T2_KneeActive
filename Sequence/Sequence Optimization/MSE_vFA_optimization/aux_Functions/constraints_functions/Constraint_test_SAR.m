...
% @IST, Uni Lisbon
% Need path of pulseq-master

function [SAR] = Constraint_test_SAR(x,ETL,params)
...
new_res   = res*accFactor;      % Resolution Needed (this also does not match because it doesn't consider the center of kspace)
TR        = params.TRini;       % (ms)
...
tend           = params.sigma2*1e-3;  % Spoilers gradients times in (s)
...
% Get time ( different from .seq because doesn't have the gradients)
aux_Trec  = (params.center_pos)*RFexcDur + TE * ETL;   % (ms)
aux_TRmin = aux_Trec + params.sigma2;                  % (ms)
TRmin     = aux_TRmin * nsli+1;                        % (ms)  - Wait for all slices to be filled out
