function [ grad , x ] = CF_Mycode_epg_derivatives_NLO_AUC( n, beta, T1, T2, alpha_exc, alpha_refoc, params, gamma)
...
% 1.1.0 - Phase of excitation and refocusing
p_rf = 0;
p_ex = pi/2;  

% 1.1.1 - RF excitation matrix, T (Rotation Matrix)
TArray{1}   =  [ (cos(alpha_exc/2))^2                  ,  exp(2*1i*p_ex)*(sin(alpha_exc/2))^2 , -1i*exp(1i*p_ex)*sin(alpha_exc);  ...
                  exp(-2*1i*p_ex)*(sin(alpha_exc/2))^2 , (cos(alpha_exc/2))^2                 ,  1i*exp(-1i*p_ex)*sin(alpha_exc); ...
                  -1i/2*exp(-1i*p_ex)*sin(alpha_exc)   ,  1i/2*exp(1i*p_ex)*sin(alpha_exc)    ,  cos(alpha_exc)];

% 1.1.2 - RF mixing matrix, T (Rotation Matrix)
for j=1:n % loop over number of echoes 
    Tzeros0 = zeros(3,3);
    T0   =  [ (cos(alpha_refoc(j)/2))^2                  ,  exp(2*1i*p_rf)*(sin(alpha_refoc(j)/2))^2 , -1i*exp(1i*p_rf)*sin(alpha_refoc(j));  ...
               exp(-2*1i*p_rf)*(sin(alpha_refoc(j)/2))^2 , (cos(alpha_refoc(j)/2))^2                 ,  1i*exp(-1i*p_rf)*sin(alpha_refoc(j)); ...
              -1i/2*exp(-1i*p_rf)*sin(alpha_refoc(j))    ,  1i/2*exp(1i*p_rf)*sin(alpha_refoc(j))    ,  cos(alpha_refoc(j))];
    % Ge in cell arrays   
    TArray{j+1}      = T0;
...
%     T0partialFAdArray = setfield(T0partialFAdArray, ['fa_',num2str(namesGenerate(j))], TArray);
    ...
    % Get all derivative
    T0d = [                    -cos(alpha_refoc(j)/2)*sin(alpha_refoc(j)/2) ,    exp(2*1i*p_rf)*sin(alpha_refoc(j)/2)*cos(alpha_refoc(j)/2) , -1i*exp(1i*p_rf) *cos(alpha_refoc(j)); ...
                exp(-2*1i*p_rf)*sin(alpha_refoc(j)/2)*cos(alpha_refoc(j)/2) ,                  -cos(alpha_refoc(j)/2)*sin(alpha_refoc(j)/2) ,  1i*exp(-1i*p_rf)*cos(alpha_refoc(j)); ...
            -1i/2*exp(-1i*p_rf)*cos(alpha_refoc(j))                   , 1i/2*exp(1i*p_rf)*cos(alpha_refoc(j))                   ,                  -sin(alpha_refoc(j))];
...
for jj=2:sizZ
    ...
    for ii=2:jj % loop over the variable
        ...
        % dx/d_partialAlpha = c's
        aux_c = dz_dalpha{1,k,l}(1);
        vector_c(k-1,l-1) = aux_c;
    end    
    
    % dy/d_beta = b
    b(jj-1) = dz_dbeta{2,jj}(1); % dy/dbeta   
    % dx/d_beta = d
    d(jj-1) = dz_dbeta{1,jj}(1); % dy/dbeta
