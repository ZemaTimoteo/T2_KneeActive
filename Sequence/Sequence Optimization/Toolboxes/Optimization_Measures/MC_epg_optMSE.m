function [uMC_fit, uMC_std] = MC_epg_optMSE(nreps, sig, Dict_norm, All_pars, sigma, T2_dic, T2, CRLB_num,GaussFit, plotTest)
% Monte Carlo study, Obtain the variance expected for a specific model/set of parameters
%
% Functions used:
%
% Inputs:
...
y          = normpdf(T2_dic,T2,-CRLB_num);     % For CRLB variance
...
%     T2plot = T2_dic(1:T2*2);
    ...
    hold on
    title(['T2 estimate histogram - SNR=',num2str(1/sigma)])
