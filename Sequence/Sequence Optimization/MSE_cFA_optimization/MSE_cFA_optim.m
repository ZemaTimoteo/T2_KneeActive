...
%% MSE optimization for constant Flip Angle (cFA)
% with Sweep Block of CRLB analysis fitting EPG curves with dEPG
...
file_path_data      = [DIR_sequenceOptimization 'Data'];
...
methodDic        = 'JUSTdict';   % For CRLB - 'SLR_Prof' OR 'JUSTdict'
...
T_acq             = 7;                    % Time max of Acquisition in min - 9min
nslices           = 15;                 % number of slices - '30'
...
vector_dTE        = 8.25:0.01:9;     % Vector of Echo SpacingTime (ms) - parecido HSa - TODO acertar ao TE possível '8:2:30'
vector_ETL        = 8:1:30;          % Vector of # Echos - ver protocolo do HSa '6:2:30'
vector_flipAngle  = 100:1:130;       % Vector of flip Angles (ms) - ver artigo tammir - '90:5:180'
vector_SNR        = [1];             % variance - % variar SNR de teste  [1 30 100] OR [1 5:15:150];
vector_T2         = [ 45 ];          % vector of different T2 values to test - [8:8:50] OR '[8 45]'

params.input.dTE = vector_dTE;
params.input.ETL = vector_ETL;
params.input.FA  = vector_flipAngle;
params.input.SNR = vector_SNR;
params.input.T2  = vector_T2;
params.input.methodDic = methodDic;
...
if writeExcel == 'True'
    ...
    cd(file_path_data)
    A1 = {'Test','ini_TR (ms)','nslices','res (mm)','sliceThickn ex (mm)','sliceThickn ref (mm)','SNR','min_T2 (ms)','max_T2 (ms)'};
    sheet = 1;
    xlRange = 'A1';
    xlswrite(filename_excell,A1,sheet,xlRange)

    A2 = {testMSEopt,TR,nslices,res,sliceThickn.exc,sliceThickn.refoc,vector_SNR(1),min(vector_T2),max(vector_T2)};
    sheet = 1;
    xlRange = ['A',num2str(testMSEopt)];
...
params             = CF_cFA_parameters(B1,T1(1),nslices,...
                            res,sliceThickn, ...
                            TR,vector_SNR(1),params); % Set parameters for Cost Function (CF)
sliceThickn.refoc  = params.st_ref; 
...
SAR_max          = 100; % W/kg
RFpower_max      = 50;  % W
maxB1_rms        = 2.55;   % uT - TODO tentar provocar os constrains
params.const.maxB1_rms = maxB1_rms;
...
maxTime_s            = maxTime * 60;    % Time in (s)
    params.const.maxTime = maxTime; 

    % 3.2.3 - Real resolution
real_res = round(res*af);
...
%                     vector_ETL(ii), vector_flipAngle(gg), sliceThickn, nslices, real_res, params,'Fals',file_path_rf);
                [b1_rms{jj,ii,gg},T_scan, TR_scan, Trec] = b1rms4seq_optMSE_cFA(vector_dTE(jj), TR, ...
                    vector_ETL(ii), vector_flipAngle(gg), sliceThickn, nslices, real_res, params,'Fals',file_path_rf);
    ...
    if saveResults == 'True'
        cd(file_path_data)
        save(['Test',num2str(testMSEopt),'_ConstrainModels_maxB1rms',num2str(maxB1_rms),'_maxTime',num2str(maxTime_s),...
...
elseif testSAR == 'loadD'    
    cd(file_path_data)
    load(['Test',num2str(testMSEopt),'_ConstrainModels_maxB1rms',num2str(maxB1_rms),'_maxTime',num2str(maxTime_s),...
          '_mindTE',num2str(min(vector_dTE)),'_maxdTE',num2str(max(vector_dTE)), ...
          '_minETL',num2str(min(vector_ETL)),'_maxETL',num2str(max(vector_ETL)),...
          '_minFlipA',num2str(min(vector_flipAngle)),'_maxFlipA',num2str(max(vector_flipAngle)),...
          '.mat'])
...
if testCRLB == 'True'
    ...
    for gg=1:size(vector_T2,2) % cycle over T2
        ...
        for jj =1:size(vector_SNR,2) % Itteration by SNR values
            ...
            for ii=1:size(Models_accepted,1) % cycle over TE
                ...
                [ vardT2(ii,jj,gg), ...
                  ds_dT2{ii,jj,gg}, ...
                  ff{ii,jj,gg} ] = CRLB_epg_optFSE_TRvar_vF(FA_exc_dic,...
        ...
        fprintf(['\n\n      Successfull CRLB model for different T2 values ',num2str(gg),...
    ...
    % save CRLB data
    cd(file_path_data)
    save(['CRLBvalues_test',num2str(testMSEopt),...
...
elseif testCRLB == 'Fals'
    % load CRLB data    
    cd(file_path_data)   
    load(['CRLBvalues_test',num2str(testMSEopt),...
          '_TRvar_ModelsAccepted',num2str(size(Models_accepted,1)),...
          '_minSNR',num2str(min(vector_SNR)),'_maxSNR',num2str(max(vector_SNR)), ...
          '_minT2-',num2str(min(vector_T2)),'_maxT2-',num2str(max(vector_T2)),...
          '.mat'])
...
% testdTE   = vector_dTE(97);          % units (ms)
% testdTE   = vector_dTE(6);           % units (ms)
% testETL   = vector_ETL(1);           % echoes
% testFlipA = vector_flipAngle(13);    % units (º) - 115

testdTE   = 8.4;           % units (ms)
testETL   = 15;           % echoes
testFlipA = 104;    % units (º) - 115
...
for gg=1:size(vector_T2,2)
    for ii=1:size(Models_accepted,1)
        new_vardT2(ii,SNRindx,gg) = vardT2(ii,SNRindx,gg)*sqrt(Models_accepted(ii,5)*60) * (1/res); % The math from SNReff/SNR = 1/sqrt/Tscan)
    end
end
...
% %     [aux, k] = (max( (Results_vardT2(:,T2indx-aux_T2indx+gg)) ) ) ;
% %     find((Results_vardT2(:,T2indx-aux_T2indx+gg))  == maxValue_vardT2(gg))
    ...
    fprintf(['   T2 max variance ' ,num2str((maxValue_vardT2(gg))),' for: ', ...
        ...
        ', B1rms = ',num2str(Results_vardT2(indxResult_vardT2,6)),' uT'])
    ...
    % write in excell the parameters
    C = {testMSEopt,log(maxValue_vardT2(gg)),vector_T2(gg),Results_vardT2(indxResult_vardT2,1), ...
            Results_vardT2(indxResult_vardT2,2),SNR_val,...
            Results_vardT2(indxResult_vardT2,3),...
            Results_vardT2(indxResult_vardT2,9)};

    if writeExcel == 'True'
        xlRange_T2 = ['A',num2str(testMSEopt)];
...
%     pcolor((results_dTE))
    ...
    xticklabels(uni_dTE_y)
    yticklabels(uni_dTE_x)
    intervPlotISMRM23 = [min(min((log(results_dTE(:,:))))) max(max((log(results_dTE(:,:)))))]; % for caxis
