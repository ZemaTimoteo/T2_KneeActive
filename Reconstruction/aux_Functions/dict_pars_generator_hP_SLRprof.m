function [dictionary,pars] = dict_pars_generator_hP_SLRprof(T1,T2,B1,ESP, ETL,refoc_phase,phase_exc,FA_exc,FA_refoc, methodDic)
...
if methodDic == 'DICT_SLR'
    ...
    for jj=1:size(B1,2)
        ...
        for ii=1:size(FA_refocUNIQUE,2)
            [exc_pulse, refoc_pulseSLR] = slr_profile(B1(jj),FA_refocUNIQUE(ii),ESP);   % corrige com os perfis adaptar para os pulsos da Siemens/pypulseq
