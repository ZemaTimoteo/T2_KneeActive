% MSE_preproc_recon_vF.m function for Phantom Images
...
plotTest     = 'True';
niftTest     = 'Fals';
maskTest     = 'True';     % 'True' or 'Fals' Mask  
allSliceTest = 'True';     % Test in all slices 'True' OR 'Fals' only one slice
LoadData     = 'Fals';     % 'True' or 'Fals' Load Data
...
DenoiseTest  = 'False';    % 'M_PCA' or 'Filtr' or 'False'
SNRmaskTest  = 'True';     % Measure the SNR within each vial
...
elseif ParallelTest == 'LORAK'
    ...
    if allSliceTest == 'Fals'
        nsli = slice;  % Selected by the user
    end

    for sli=iniSlice:nsli
        for echo = 1:ETL
...
if LoadData == 'True'
    ...
    elseif ParallelTest == 'LORAK'
        if AllSave == 'True'        
            load('LORAKS_recon_matlab.mat')
        else
            load('LORAKS_recon_matlab_justImag.mat')
            recon_Img_full = permute(recon_Img_full,[2 1 3 4]);            
        end        
    end

    if AllSave == 'True'
        %auxiliar rearrange data
...
%             for echo=1:ETL
            for echo=1:ETL
                % 3.0 - Get kdata
                kdata = sig_red(:,[[1:iniKfull+1] [iniKfull+2:af:endKfull+2] [endKfull+1+2:orgSizeNx]],:,sli,echo); % the plus 1 is because of python index, the +2 is to get new position
                ...
                fprintf(['  --  GRAPPA recon for echo ',num2str(echo),' / ',num2str(ETL),'\n\n'])            
            end
        end
        ...
        for sli=iniSlice:nsli
            for coil=1:ncoils
                ...
                elseif LORAKStest == 'ACloraks'
                    sigrecon(coil,:,:,sli,:) = AC_LORAKS( squeeze(kdata), squeeze(mask), rankLORAKS );  % Under sampled Data, Mask, Rank
                end    
            ...
            for echo=1:ETL
                ...
                % 3.5 - Get data from all coils
                recon_Img_full(:,:,sli,echo) = squeeze(sum(abs(aux_img_PI(:,:,:,sli,echo).^2),1).^.5);  % LS method for coil combination % size(aux_img_PI) = 18 258 256 3 12
    % %             recon_Img_full(:,:,sli,echo) = squeeze(abs(sum(aux_img_PI(:,:,:,sli,echo).*conj(aux_img_PI(:,:,:,sli,echo)),ndims(aux_img_PI(:,:,:,sli,echo)))).^(1/2));
    ...
    elseif ParallelTest == 'GRAPP'
        if AllSave == 'True'        
            ...
            recon_Img_full = permute(recon_Img_full,[2 1 3 4]);            
            save('GRAPPA_recon_matlab_justImag.mat','recon_Img_full')
            recon_Img_full = permute(recon_Img_full,[2 1 3 4]);
        end
    ...
    elseif ParallelTest == 'LORAK'
        if AllSave == 'True'
            ...
            recon_Img_full = permute(recon_Img_full,[2 1 3 4]);
            save('LORAKS_recon_matlab_justImag.mat','recon_Img_full')
            recon_Img_full = permute(recon_Img_full,[2 1 3 4]);
        end
...
if plotTest == 'True'
    ...
    if AllSave == 'True'
        ...
        fig1 = figure('Name','Results','WindowState','maximized');
        imshow(recon_Img_full(:,:,sli,1),[]);           % display image 1st echo random slice with good cartilage display
        saveas(fig1,'Recon_image_&_echos.png')
        fig2 = figure();
        for sli =1:nsli
            plotImageRecon = squeeze(recon_Img_full(:,:,sli, 1));  % LS method for coil combination
            subplot(4,4,sli)
            imagesc(plotImageRecon,[0 .4])
            hold on, title(['Slice = ',num2str(sli)])
            colormap gray
        ...
        saveas(fig2,'Recon_image_&_AllSlices.png')
...
%     figure()
%     plot(squeeze(recon_Img_full(137,248,:,:)))
end
...
if maskTest == 'True'
    ...
    for sli=1:1
        ...
        for jj=1:nvials
            % --  Circular Vial mask     
            T2_mask_m = zeros(size(recon_Img_full(:,:,sli)));
            figure('Name','Results','WindowState','maximized'); imshow(recon_Img_full(:,:,sli,3),[]);           % display image 1st echo random slice with good cartilage display
            ...
            T2_mask(:,:,jj,sli) = double(T2_mask_m);
...
if SNRmaskTest == 'True'
    for sli=1:1
        nvials = 14;
        for jj=1:nvials
            aux_Data =  T2_mask(:,:,jj) .*  recon_Img_full(:,:,sli,1);
            SignalVial(jj) = mean(nonzeros( aux_Data ));
            SNRVial(jj)    = std(nonzeros(aux_Data));
            SNR(jj) = SignalVial(jj) / SNRVial(jj);            
        end
    end
    snr = SNR'
    save(['SNRperVial'],'SNR')   
    ...
    %% -------------- 6 - Denoise image ------------------------------
if DenoiseTest == 'Filtr'

    echoFilt = 1;
    data_filter_test = squeeze(recon_Img_full(:,:,1,echoFilt));
    % 5.0 - normal
    figure()
    subplot(221)
    imagesc(data_filter_test)
    colormap gray
    hold on, title(['Original'])
    caxis([ 0   max( max(data_filter_test) ) ])

    % 5.1 - Gaussian filter  Image Space
    sigma = 2;
    testIMG_1 = imgaussfilt(data_filter_test,sigma);
    subplot(222)
    imagesc(testIMG_1)
    colormap gray
...
% % % v = unring(v,params);
    ...
    % ... 6.2 - Denoise with MP-PCA 
elseif DenoiseTest == 'M_PCA'
    % 6.2.1 - Set parameters
    image  = squeeze(recon_Img_full);
    Janela   = [7,7]; % from Diffusion MRI noise mapping using random matrix theory
    mask   = [];

    % 6.2.2 - MP-PCA denoising - Matlab implementation of Marchenko Pastur denoising (Veraart et al, NeuroImage 142 (2016) 394–406)
    [denoised,~,~] = denoise(image,Janela);

    % 6.2.3 - Figures
    figure()
    subplot(221)
    imagesc(squeeze(image(:,:,1)))
    colormap gray
    hold on, title(['Original'])
    caxis([ 0   max( max(data_filter_test) ) ])        
    subplot(222)
    imagesc(squeeze(denoised(:,:,1)))
    colormap gray
    hold on, title(['Denoised'])
    caxis([ 0   max( max(data_filter_test) ) ])
    subplot(223)
    imagesc(squeeze(image(:,:,1) - denoised(:,:,1)))
    colormap gray
    hold on, title(['Diff'])

    subplot(224)
    plot(squeeze(image(156,237,:)))
    hold on, title(['Decay of signals [237,156]'])
    plot(squeeze(denoised(156,237,:)))
...
if niftTest == 'True'
    mkdir([data_dir '\imagResults_orig'])
    % Combine coils
    if AllSave == 'True'
