...
from slr_profile_seq import slr_profile_seq

def dict_pars_generator_seq(T1,T2,B1,ESP, ETL,refoc_phase,phase_exc,FA_exc,FA_refoc, dir_rf, SLR=True):
    ...
    for jj in range(B1.shape[0]):
        ...
        if SLR:
            for ii in range(FA_refoc.shape[0]): # only accept one FA_refoc
                [exc_pulse, val_refoc_pulse] = slr_profile_seq(FA_exc, 1, FA_refoc[ii], dir_rf) # corrige com os perfis adaptar para os pulsos da Siemens/pypulseq
        ...
        # ... 2.2 - slice profile without considering SLR profile ...
            ...
            val_refoc_pulse = float(FA_refoc) * B1[jj] * (math.pi/180)
